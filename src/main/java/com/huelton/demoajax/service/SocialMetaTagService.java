	...
	private SocialMetaTag getTwitterCardByUrl(String url){
		...
		try {
			Document doc = Jsoup.connect(url)
				    .header("Access-Control-Allow-Origin", "https://collector-pxzhh9f9x0.perimeterx.net/api/v1/collector")
				    .get();
			tag.setTitle(doc.head().select("meta[property=twitter:title]").attr("content"));
			tag.setSite(doc.head().select("meta[property=twitter:site]").attr("content"));
			tag.setImage(doc.head().select("meta[property=twitter:image]").attr("content"));
			tag.setUrl(doc.head().select("meta[property=twitter:url]").attr("content"));
		} catch (IOException e) {
